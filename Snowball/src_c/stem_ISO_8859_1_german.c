...
static int r_standard_suffix(struct SN_env * z) {
	int among_var;
	{
		...
		case 2: {
			...
			break;
		case 3:
			if (in_grouping_b(z, g_s_ending, 98, 116, 0))
				goto lab0;
			{
	...
	}
	{
		...
		case 1: {
			...
			break;
		case 2:
			if (in_grouping_b(z, g_st_ending, 98, 116, 0))
				goto lab2;
			{
				int ret = z->c - 3;
				if (z->lb > ret || ret > z->l)
